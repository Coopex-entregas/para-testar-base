...
def admin_novo_socorro():
    ...
    IMPORTANTE: aqui NÃO marcamos como lido — o socorro só é encerrado quando o admin clicar no X
    (ver rota /admin_socorro_marcar_lido).
    """
    ...
    return jsonify({
        "novo": True,
        "cooperado": ULTIMO_SOCORRO.get("cooperado_nome", ""),
        "mensagem": ULTIMO_SOCORRO.get("mensagem", "") or "",
        "momento": ULTIMO_SOCORRO.get("momento", ""),
    }), 200


@app.route("/admin_socorro_marcar_lido", methods=["POST"])
def admin_socorro_marcar_lido():
    """Marca o socorro atual como lido (admin clicou no X)."""
    if not session.get("is_admin") and not session.get("is_master"):
        abort(403)
